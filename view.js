function show() {
    ...
    for (let i = 0; i < model.player.superSmash.attacks.attackNames.length; i++) {
        attacks +=`
        <button onclick="playerAttack(${model.player.superSmash.attacks.power[i]})">
    ...
    let html = `
        ...
        <img src="${model.boss.superSmash.image()}"> <br>
           ...
           ${model.attackMovePower  == "" ? "" :`${model.boss.superSmash.name} used ${model.attack}!<br> it attacked with ${model.attackMovePower} damage!`}
        </div>     
    `;
    app.innerHTML = html;
}